...
<center>
  ...
  src="../man/figures/workflow_nonetwork.png"
  alt="Workflow of the bioregion package for hierarchical clustering.">
</center> 
...
We can also see directly in the console the hierarchical structure of the tree,
with info on each cluster, by using the convenient `summary()` function.
...
Let's look at the hierarchical structure of clusters with `summary()`:
```{r}
summary(tree1)
```
...
<center>
  <img align="bottom" width="100%" height="100%" 
   src="../man/figures/find_optimal_n.png"
   alt="How to find an optimal number of clusters?">
</center> 
...
which compares the total dissimilarity with the inter-cluster dissimilarity (sum 
of distances between clusters). Then we will choose the optimal number of 
clusters as the elbow of the evaluation plot.
...
Currently, there are four evaluation metrics available in the package described
[here](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html#bioregionalization).

**Important note**

To be able to calculate `prop_between_dissim` and `anosim`, you need to provide
your dissimilarity object to the argument `dissimilarity`. In addition, to be
able to calculate `mean_endemics` and `tot_endemics`, you need to provide your
species-site co-occurrence matrix to the argument `comat`.
...
L-shaped form (typically, `prop_between_dissim` and endemism metrics), but not
for other metrics (e.g. the form of `anosim` does not necessarily follow an L-shape).
...
metric, from a minimum of `r min(unlist(a$optimal_n))` to a maximum
of `r max(unlist(a$optimal_n))`. The final choice depends on your
...
metrics e.g. at a value of `r a$optimal_n$mean_endemics`, and a
shallow cutoff based on `prop_between_dissim`,
at `r a$optimal_n$prop_between_dissim`.
...
`increasing_step` is for increasing metrics (`prop_between_dissim`) and 
`decreasing_step` is for decreasing metrics (`Mean_Endemics` and 
`tot_endemics`). `anosim` values can either increase or decrease depending on
...
used different cutoffs for `BetweenDissim` to find the global biogeographic 
