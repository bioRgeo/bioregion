...
Before diving into specific metrics, we can understand the core logic using a 
...
### When clusters are assigned to sites (`node_type = "site"` or `node_type = "both"`) 
...
| EndemicRichness | Site | Bioregion | Co-occurrence| ✓ | — | $E_g = \sum{K_s}$ | Number of endemic species in the site (i.e., species occurring in only one bioregion) |
| pctEndemic | Site | Bioregion | Co-occurrence | ✓ | — | $\bar{PctEnd}_{g} = \frac{E_g}{S_g}$ | Percentage of endemic species in the site |
| | | | | | | | |
...
### When clusters are assigned to species (`node_type = "species"` or `node_type = "both"`) 
...
| | | | | | | | |
| Richness | Site | — | Co-occurrence | ✓ | — | $S_g = n_g$ | Number of species |
| chorotypeRichness | Site | — | Co-occurrence | ✓ | — | $S_c = n_gc$ | Number of species assigned to each cluster |
...
### When `node_type = "species"` (or `"both"`)
...
These metrics are computed when sites have clusters (i.e., `node_type = "site"`
...
- if the clustering is based on sites (`node_type = "site"`
(or `"both"`)), then richness and similarity-based metrics can be computed
- if the clustering is based on species (`node_type = "species"`
...
- EndemicRichness = number of species in the site that are endemic to a single region (i.e., occur on only one bioregion)
- pctEndemic = percentage of endemic species, i.e. ratio between EndemicRichness and Richness 
...
of species clusters (`node_type = "species"`).
...
These indices measure whether a site has species from a single region or from 
