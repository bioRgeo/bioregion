...
In this vignette, we describe three functions to compute summary metrics:
...
- metrics calculated for a bioregionalization `bioregionalization_metrics()`
...
**IndVal** (IndVal = Specificity × Fidelity):
...
| n_specificity | Species | Bioregion | Co-occurrence | ✓ | ✓ | $\bar{A}_{sb} = \frac{n_{sb}/n_b}{\sum_k n_{sk}/n_k}$ | Size-normalized specificity |
| fidelity | Species | Bioregion | Co-occurrence | ✓ | ✓ | $B_{sb} = \frac{n_{sb}}{n_b}$ | Fraction of bioregion's sites with species |
| ind_val | Species | Bioregion | Co-occurrence | ✓ | ✓ | $A_{sb} \times B_{sb}$ | Indicator value (specificity × fidelity) |
| n_ind_val | Species | Bioregion | Co-occurrence | ✓ | ✓ | $\bar{A}_{sb} \times B_{sb}$ | Size-normalized indicator value |
| rho | Species | Bioregion | Co-occurrence | ✓ | ✓ | See section 7.1.1 | Standardized contribution index |
| core_terms | Species | Bioregion | Co-occurrence | ✓ | ✓ | $n$, $n_b$, $n_s$, $n_{sb}$ | Raw counts for custom calculations |
| | | | | | | | |
...
| | | | | | | | |
| mean_sim | Site | Bioregion | Similarity | — | — | $\frac{1}{n_b - \delta} \sum_{g' \neq g} sim_{gg'}$ | Mean similarity to bioregion |
...
| n_specificity | Site | Chorotype | Co-occurrence | ✓ | ✓ | $\bar{A}_{gc} = \frac{n_{gc}/n_c}{\sum_k n_{gk}/n_k}$ | Size-normalized specificity |
| fidelity | Site | Chorotype | Co-occurrence | ✓ | ✓ | $B_{gc} = \frac{n_{gc}}{n_c}$ | Fraction of cluster's species in site |
| ind_val | Site | Chorotype | Co-occurrence | ✓ | ✓ | $A_{gc} \times B_{gc}$ | Indicator value (specificity × fidelity) |
| n_ind_val | Site | Chorotype | Co-occurrence | ✓ | ✓ | $\bar{A}_{gc} \times B_{gc}$ | Size-normalized indicator value |
| rho | Site | Chorotype | Co-occurrence | ✓ | ✓ | See section 7.2.2 | Standardized contribution index |
| core_terms | Site | Chorotype | Co-occurrence | ✓ | ✓ | $n$, $n_c$, $n_g$, $n_{gc}$ | Raw counts for custom calculations |
...
You can also run `summary()` on the object to quickly see a 
...
We can see it also displays the top sites or species for ind_val for a convenient
...
(`bioregion_metrics = c("specificity", "n_specificity", "fidelity", "ind_val", "n_ind_val",
"rho", "core_terms")`). To compute these metrics, we need to provide `comat`.
...
The indicator value ${IndVal}_{sb}$ of species $s$ for bioregion $b$ can be 
defined as the product of specificity and fidelity [@Caceres2009]:

$${IndVal}_{sb} = A_{sb} \times B_{sb}$$
...
$${IndVal}_{sb} = A_{sb} \times \frac{n_{sb}}{n_b}$$
...
bioregion $b$ (as in n_specificity and n_ind_val).
...
at the species level, such as affinity, fidelity, ind_val and other similar metrics.
...
each bioregion ($\mathrm{mean\_sim}$) and the associated standard deviation 
($\mathrm{sd\_sim}$). When computing the average similarity, the focal site 
itself is not included in the calculation for its own bioregion.
...
# 9. Bioregion metrics & spatial coherence {#bioregion}

At the bioregion level, we can compute the number of sites and the total  
species richness across those sites. We can also derived the number and  
proportion of endemic species, defined (as above) as species occurring exclusively  
...
We also use the metric of spatial coherence following @Divisek2016.  
Spatial coherence quantifies how contiguous the sites of a bioregion are.  
Two variants are computed:

- **sc_size**: the fraction of the bioregion's sites contained in its largest  
  contiguous patch, calculated based on site counts rather than area.

- **sc_area**: the fraction of the bioregion's total area contained in its  
  largest spatially contiguous patch, calculated from polygon geometries.

Both metrics range from 0 to 1, with 1 indicating a fully contiguous bioregion.
...
The bioregion 1 is almost constituted of one homogeneous block, which is why 
the spatial coherence is very close to 1.
...
# 10. Bioregionalization metrics {#bioregionalization}

At the bioregionalization level, several metrics described below can be
computed to assess the quality of the resulting bioregionalization across
its different partitions. As is the case for `site_species_metrics()`, some
metrics are based on a similarity or dissimilarity object, while others are
based on a co-occurrence matrix.
...
This metric, proposed by @Holt2013, computes the proportion of total
dissimilarity occurring between bioregions relative to the total
dissimilarity. Following the same notation, the metric can be defined as


$$
\frac{
\sum_{g,g'} dissim_{gg'}
\left(1 - \sum_b \delta_{g \in b}\delta_{g' \in b}\right)
}{
\sum_{g,g'} dissim_{gg'}
}
$$

where $dissim_{gg'}$ is the dissimilarity between sites $g$ and $g'$.
...
The Analysis of Similarities (ANOSIM) statistic
measures the separation between within- and between-bioregion
dissimilarities. It is based on the ranks of the dissimilarities and
compares the average ranks within bioregions to those between bioregions.
...
When `permutation > 1`, a p-value is calculated using permutation testing.
...
The statistic and associated p-value are computed using the [anosim](https://vegandevs.github.io/vegan/reference/anosim.html#references) 
function from the R package 
[vegan](https://cran.r-project.org/package=vegan). 

### mean_endemics

The mean endemism measures the average proportion of endemic species across
bioregions, as proposed by @Kreft2010. It is calculated as the mean of the
proportion of endemic species within each bioregion. 
...
This information is directly derived from the ouptput of \code{bioregion_metrics}
...
The proportion of endemic species across all bioregion. It is calculated as the 
total number of endemic species divided by the total number of species. Endemic 
species are those occurring in only one bioregion.
