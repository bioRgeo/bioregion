...
The `bioRgeo`'s package contains as example dataset the spatial distribution of 
Mediterrean vegetation. This dataset has been analyzed in [this article](https://onlinelibrary.wiley.com/doi/full/10.1002/ece3.4718) and contains
the abundance of 3,697 species in 715 sites. This dataset is composed of three 
files, [vegedf](https://biorgeo.github.io/bioRgeo/reference/vegedf.html) a 
...
a co-occurence `matrix` containing the same information gathered in a `matrix`
...
and [vegesp](https://biorgeo.github.io/bioRgeo/reference/vegesp.html) a spatial
object containing the geometry of the 715 sites. 

## From matrix to network

The function [mat_to_net](https://biorgeo.github.io/bioRgeo/reference/mat_to_net.html) 
transforms a co-occurence `matrix` such as `vegemat` into a network represented 
...
The function [net_to_mat](https://biorgeo.github.io/bioRgeo/reference/net_to_mat.html)
does the opposite. It transforms a network represented by a two- or a three-columns
`data.frame` (such as `vegedf`) into a co-occurence `matrix` (such as `vegemat` 
in this case). 
