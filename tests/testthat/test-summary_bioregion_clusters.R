...
test_that("summary works for non-hierarchical clustering (nhclu_pam)", {
  ...
  clust_pam <- nhclu_pam(dissim_fish, 
                         n_clust = 5, 
                         seed = 123)
  ...
  expect_no_error(quietly(summary(clust_pam, 
                                  n_bioregionalizations = 1, 
                                  n_top_clusters = 3)))
  expect_no_error(quietly(summary(clust_pam, 
                                  n_bioregionalizations = 10, 
                                  n_top_clusters = 20)))
...
test_that("summary works for hierarchical clustering (hclu_hierarclust)", {
  ...
  clust_hclust <- hclu_hierarclust(dissim_fish,
                                   n_clust = c(3, 10, 50),
...
test_that("summary works for unipartite netclu_infomap without hierarchy", {
  ...
  clust_infomap_uni_flat <- netclu_infomap(sim_fish,
                                           bipartite = FALSE,
  ...
  # Test that summary returns the object invisibly
  quietly( 
    result <- summary(clust_infomap_uni_flat, 
                    n_bioregionalizations = 3, 
                    n_top_clusters = 10)
  )
...
test_that("summary works for unipartite netclu_infomap with hierarchy", {
  ...
  clust_infomap_uni_hier <- netclu_infomap(sim_fish,
                                           bipartite = FALSE,
                                           bipartite_version = FALSE,
                                           show_hierarchy = TRUE,
                                           seed = 123)
  ...
  # Test that summary runs without error
  expect_no_error(quietly(summary(clust_infomap_uni_hier)))
  
  # Test that summary returns the object invisibly
  quietly(
    result <- summary(clust_infomap_uni_hier, 
                      n_bioregionalizations = 3, 
                      n_top_clusters = 10)
  )
...
#   
#   clust_infomap_bip_flat <- netclu_infomap(sim_fish,
#                                            bipartite = TRUE,
#                                            bipartite_version = FALSE,
#                                            show_hierarchy = FALSE,
#                                            nbmod = 5,
...
#   
#   # Test that summary runs without error
#   expect_no_error(quietly(summary(clust_infomap_bip_flat)))
#   
#   # Test that summary returns the object invisibly
#   quietly(
#     result <- summary(clust_infomap_bip_flat, 
#                       n_bioregionalizations = 3, 
#                       n_top_clusters = 10)
#   )
...
# 
#   clust_infomap_bip_hier <- netclu_infomap(sim_fish,
#                                            bipartite = TRUE,
#                                            bipartite_version = FALSE,
#                                            show_hierarchy = TRUE,
#                                            seed = 123)
#   
#   # Test that summary runs without error
#   expect_no_error(quietly(summary(clust_infomap_bip_hier)))
#   
#   # Test that summary returns the object invisibly
#   quietly(
#     result <- summary(clust_infomap_bip_hier, 
#                       n_bioregionalizations = 3, 
#                       n_top_clusters = 10)
#   )
...
test_that("summary handles edge cases correctly", {
  ...
  expect_no_error(quietly(summary(clust_pam, n_bioregionalizations = 100)))
  
  # Test with n_top_clusters larger than available
  expect_no_error(quietly(summary(clust_pam, n_top_clusters = 1000)))
  
  # Test with n_bioregionalizations = 0 or negative (should show at least available)
  expect_no_error(quietly(summary(clust_pam, n_bioregionalizations = 0)))
})

# Test 9: Display hierarchy function -------------------------------------------
test_that("display_hierarchy function works correctly", {
  
  clust_infomap_hier <- netclu_infomap(sim_fish,
                                       bipartite = FALSE,
                                       bipartite_version = FALSE,
                                       show_hierarchy = TRUE,
                                       seed = 123)
  ...
  quietly(
    output <- capture.output(summary(clust_infomap_hier, 
                                     n_bioregionalizations = 3))
  )
