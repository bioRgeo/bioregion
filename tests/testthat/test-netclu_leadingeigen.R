...
test_that("number of columns in output", {
  clust1 <- netclu_leadingeigen(net)
  
  expect_equal(clust1$name, "netclu_leadingeigen")
  ...
  expect_equal(clust2$name, "netclu_leadingeigen")
...
test_that("error messages with wrong inputs", {
  expect_error(
    netclu_leadingeigen(net = "zz"),
    "net must be a data.frame.",
    fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net, weight = "zz"),
    "weight must be a boolean.",
    fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net, index = "zz"),
    "index is a character, it should be a column name (and not the
                    first or second column).",
    fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net, bipartite = "zz"),
    "bipartite must be a boolean",
    fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net_bip, bipartite = TRUE, site_col = "zz"),
    "If site_col is a character, it should be the first or second column name."
    , fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net_bip, bipartite = TRUE, species_col = "zz"),
    "If species_col is a character, it should be the first or second column name."
    , fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net_bip, bipartite = TRUE, return_node_type = "zz"),
    "Please choose return_node_type among the followings values:
both, sites and species",
    fixed = TRUE)
  
  expect_error(
    netclu_leadingeigen(net = net, algorithm_in_output = "zz"),
