...
test_that("valid output", {
  ...
  clust <- netclu_labelprop(net, bipartite = TRUE)
  expect_equal(dim(clust$clusters)[1], 7)
  expect_equal(clust$args$return_node_type, "both")
  
  clust <- netclu_labelprop(net, 
                         bipartite = TRUE, 
  ...
  clust <- netclu_labelprop(net, bipartite = TRUE, weight = TRUE)
  expect_equal(clust$inputs$data_type, "abundance")
  
  # Test data_type with bipartite network (unweighted)
  clust <- netclu_labelprop(net, bipartite = TRUE, weight = FALSE)
  expect_equal(clust$inputs$data_type, "occurrence")
  
  # Test data_type with similarity metrics (occurrence-based)
  clust <- netclu_labelprop(simil, index = "Jaccard")
  expect_equal(clust$inputs$data_type, "occurrence")
  
  clust <- netclu_labelprop(simil, index = "Simpson")
  expect_equal(clust$inputs$data_type, "occurrence")
  
  # Test data_type with similarity metrics (abundance-based)
  clust <- netclu_labelprop(simil, index = "Bray")
  expect_equal(clust$inputs$data_type, "abundance")
...
test_that("invalid inputs", {
  ...
  expect_error(
    netclu_labelprop(net, bipartite = 1),
    "bipartite must be a boolean.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = c("zz","zz")),
    "bipartite must be of length 1.",
    fixed = TRUE)
  
#   expect_message(
#     netclu_labelprop(net, bipartite = FALSE),
...
#     fixed = TRUE)
  ...
  expect_error(
    netclu_labelprop(net[,-3], weight = TRUE),
    "^net must be a data.frame with at least three columns if weight ")
  
  expect_error(
    netclu_labelprop(net, index = c("zz",1)),
    "index must be of length 1.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, index = "zz"),
    "^If index is a character, it should be a column name ")
  
  expect_error(
    netclu_labelprop(net, index = "Site1"),
    "^If index is a character, it should be a column name")
  
  expect_error(
    netclu_labelprop(net, index = 0.1),
    "If index is numeric, it should be an integer.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, index = 2),
    "index should be strictly higher than 2.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, index = 4),
    "index should be lower or equal to 3.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(simil, index = 1),
    "index should be strictly higher than 2.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(simil, index = 16),
    "index should be lower or equal to 15.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net5, weight = TRUE),
    "NA(s) detected in the weight column.", 
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net2, weight = TRUE),
    "The weight column must be numeric.", 
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net6, weight = TRUE),
    "The weight column should contain only positive values.", 
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net6, weight = TRUE, index = 3),
    "The weight column should contain only positive values.", 
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net6, weight = TRUE, index = "Weight"),
    "The weight column should contain only positive values.", 
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net6, weight = TRUE, index = 5),
    "The weight column should contain only positive values.", 
    fixed = TRUE)
  ...
  expect_error(
    netclu_labelprop(uni, bipartite = TRUE),
    "The network is not bipartite!", 
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, return_node_type = 1),
    "return_node_type must be a character.",
    fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, return_node_type = "zz"),
    "^Please choose return_node_type from the following:")
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, site_col = "Weight", 
                  species_col = "Weight"),
    "site_col and species_col should not be the same."
    , fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, site_col = "zz"),
    "If site_col is a character, it should be the first or second column name."
    , fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, species_col = "zz"),
  ...
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, site_col = 3),
    "If site_col is numeric, it should be equal to 1 or 2."
    , fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, site_col = FALSE),
    "site_col should be numeric or character."
    , fixed = TRUE)
  
  expect_error(
    netclu_labelprop(net, bipartite = TRUE, species_col = FALSE),
