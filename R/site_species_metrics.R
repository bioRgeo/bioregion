...
#' categories are available: species to bioregions (`"Affinity"`, `"Fidelity"`,
#' `"IndVal"`, `"Rho"` and `"CoreTerms"`), site to bioregions (`"MeanSim"` and 
...
#' be considered to compute the related indices (`"Affinity"`, `"Fidelity"`,
...
#'  indices (`"Affinity"`, `"Fidelity"`, `"IndVal"`, `"Rho"` and `"CoreTerms"`) 
...
#' contribution of each species to a given bioregion. This is calculated from 
#' the bioregion assigned to each site (as defined in `bioregionalization`) and 
#' an associated site–species co-occurrence matrix `comat` (a strict match 
#' between site IDs is required). 
#' 
#' Occurrence-based indices are derived from three core terms:
#' 
...
#' 
#' - [**Affinity**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Bernardo-Madrid *et al.* (2019).  
#' - [**Fidelity**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Bernardo-Madrid *et al.* (2019).  
#' - [**IndVal**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Bernardo-Madrid *et al.* (2019).  
#' - [**Rho**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Lenormand *et al.* (2019).  
#' 
#' **Species-to-bioregionalization** indices can also be computed, such as:  
#' 
#' - [**Participation**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Denelle *et al.* (2020) and originally introduced by 
#'   Guimerà & Amaral (2005).  
#' 
#' Abundance-weighted versions of these indices can also be derived using the 
#' following analogous core terms:  
#' 
...
#' counterparts and allow computing abundance-sensitive versions of the same 
...
#' contribution of each site to a given *species cluster* (only when a 
...
#' 
#' In this case, occurrence-based indices are derived from three core terms:
#' 
...
#' versions) can be derived from these **site-to-clusters** relationships. Further 
#' details, mathematical definitions, and examples are provided in the package 
#' vignette.
#' 
#' The **third type** of indices provided by this function is based on the 
#' contribution of each site to a given bioregion. This is calculated from 
#' the bioregion assigned to each site (as defined in `bioregionalization`) and 
#' a site–site similarity metric (`similarity`) (a strict match between site 
#' IDs is required). 
#' 
#' These **site-to-bioregion** indices include:  
#' 
#' - [**MeanSim**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html): 
#'   The mean similarity of each site to the sites of every bioregion.  
#' - [**SdSim**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html): 
#'   The corresponding standard deviation of similarity values.  
#' 
#' **Site-to-bioregionalization** indices can also be computed, such as:  
#' 
#' - [**Silhouette**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Rousseeuw (1987) (similarity-based version).  
#' - [**Ratio**](https://biorgeo.github.io/bioregion/articles/a5_2_summary_metrics.html), 
#'   as described in Lenormand *et al.* (2019).  
#' 
#' As for the previous types, all indices—including their abundance-weighted 
#' versions—can also be derived from this site-to-bioregion approach. Detailed 
#' formulas, derivations, and examples are provided in the package vignette.
#'
#' @references
...
site_species_metrics <- function(bioregionalization,
  ...
  if(type == "gb" | type == "both"){ # site-to-bioregions (gb) included
    if(b_node_type == "species"){
      stop(paste0("Site-to-bioregions metrics are not available ",
  ...
  if(type == "sb" | type == "both"){ # species-to-bioregions (sb) included
    ...
    if(b_node_type == "site" & node_type != "site"){
      stop(paste0("Site-to-species_clusters metrics are not available ",
  ...
  if(type == "sb"){ # species-to-bioregions (sb) only
    ...
    if(b_node_type == "species" & node_type != "species"){
      stop(paste0("Species-to-bioregions metrics are not available ",
                  "when no bioregion are assigned to the site in ",
                  "bioregionalization."), 
           call. = FALSE)
    }
