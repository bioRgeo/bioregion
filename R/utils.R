  ...
  if (type == "input_similarity") {
    if (inherits(data, "bioRgeo.pairwise.metric")) {
      if (attr(data, "type") == "dissimilarity") {
        stop(paste0(deparse(substitute(data)), " seems to be a dissimilarity object. 
...
Use dissimilarity_to_similarity() before using this function."), call. = FALSE)
      ...
      message(paste0(deparse(substitute(data)), " is not a bioRgeo.pairwise.metric object. 
Note that some functions required dissimilarity metrics (hclu_ & nhclu) and others similarity metrics (netclu_). 
Please carefully check your data before using the clustering functions."), call. = FALSE)
    }
  }
...
Please carefully check your data before using the clustering functions."))
  ...
  if (type == "input_net") {
    ...
    if (dim(data)[2] < 2) {
      stop(paste0(deparse(substitute(data)), " must be a data.frame with at least two columns."), call. = FALSE)
    }
    pairs1 <- paste0(data[, 1], "_", data[, 2])
    if (sum(duplicated(pairs1)) > 0) {
      stop(paste0(
        "The first two columns of ", deparse(substitute(data)), " contain duplicated pairs of nodes!"
      ), call. = FALSE)
    }
    nbna <- sum(is.na(data))
    if (nbna > 0) {
  ...
  if (type == "input_net_directed") {
    ...
    if (!args) {
      if (length(intersect(pairs1, pairs2)) > 0) {
        stop(paste0(deparse(substitute(data)), " should not be directed if directed = FALSE."), call. = FALSE)
      }
    }
  }
  ...
  if (type == "input_net_weight") {
    ...
    if (args & dim(data)[2] == 2) {
      stop(paste0(deparse(substitute(args)), " must be a data.frame with at least three columns if weight equal TRUE."), call. = FALSE)
    }
  }
  ...
  if (type == "input_net_index") {
    if (is.character(args)) {
      if (!(args %in% colnames(data)[-(1:2)])) {
        stop(paste0(deparse(substitute(args)), " is a character, it should be a column name (and not the first or second column)."), call. = FALSE)
      }
    } else if (is.factor(args)) {
      args <- as.character(args)
      if (!(args %in% colnames(data)[-(1:2)])) {
        stop(paste0("If ", deparse(substitute(args)), " is a character, it should be a column name (and not the first or second column)."),
          call. = FALSE
        )
      }
    } else if (is.numeric(args)) {
      if (args %% 1 != 0) {
        stop(paste0("If ", deparse(substitute(args)), " is numeric, it should be an integer."), call. = FALSE)
      } else {
        if ((args <= 2)) {
          stop(paste0(deparse(substitute(args)), " should be stricltly higher than 2."), call. = FALSE)
        }
        if ((args > dim(data)[2])) {
          stop(paste0(deparse(substitute(args)), " should be lower or equal to ", dim(data)[2], "."), call. = FALSE)
        }
      }
    } else {
      stop(paste0(deparse(substitute(args)), " should be numeric or character."), call. = FALSE)
    }
  }
  ...
  if (type == "input_net_index_value") {
    if (!is.numeric(data[, 3])) {
      ...
      minet <- min(data[, 3])
      if (minet < 0) {
        stop("The (dis)similarity metric should contain only positive reals: negative value(s) detected!", call. = FALSE)
      }
    }
  }
  ...
  if (type == "input_net_bip_col") {
    if (is.character(args)) {
      if (!(args %in% colnames(data)[1:2])) {
        stop(paste0("If ", deparse(substitute(args)), " is a character, it should be the first or second column name."), call. = FALSE)
      }
    } else if (is.factor(args)) {
      args <- as.character(args)
      if (!(args %in% colnames(data)[1:2])) {
      ...
      if ((args != 1) & (args != 2)) {
        stop(paste0("If ", deparse(substitute(args)), " is numeric, it should be equal to 1 or 2."), call. = FALSE)
      }
    } else {
      stop(paste0(deparse(substitute(args)), " should be numeric or character."), call. = FALSE)
    }
  ...
  if (type == "input_matrix") {
    if (!is.matrix(data)) {
      stop(paste0(deparse(substitute(data)), "must be a matrix."), call. = FALSE)
    }
  ...
  if (type == "strict_positive_numeric") {
    if (!is.numeric(args)) {
      ...
      if (args <= 0) {
        stop(paste0(deparse(substitute(args)), " must be strictly higher than 0."), call. = FALSE)
      }
    }
  }
