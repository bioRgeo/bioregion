...
plot.bioRgeo.hierar.tree <- function(x, ...)
{
  ...
  if(!is.null(x$output_cut_height))
  {
    ...
    if(length(x$output_cut_height) > 1)
    {
      ...
      for(i in 1:min(3, length(x$output_cut_height)))
      {
        stats::rect.hclust(x$final.tree,
                           h = x$output_cut_height[i],
                           border = cols[i])
      }
    ...
    {
      stats::rect.hclust(x$final.tree,
                         h = x$output_cut_height,
                         border = "#377eb8")
    }
  } else if(x$args$dynamic_tree_cut)
  {
    # Adding rectangles for dynamic tree cut
    vect_clust <- x$clusters$cluster
    names(vect_clust) <- x$clusters$site
    tot_l <- x$output_n_clust + length(which(is.na(vect_clust)))

    vect_clust[is.na(vect_clust)] <- (x$output_n_clust + 1):
      (x$output_n_clust + length(which(is.na(vect_clust))))

    order_rect <- unique(vect_clust[x$final.tree$order])

    true_cl <- which(order_rect %in% 1:x$output_n_clust)
